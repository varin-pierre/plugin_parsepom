<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
            ...
            <xsl:choose>			
                <xsl:when test="not(string(display-portlet-title)='1')">
                    ...
                    <div class="portlet-content -lutece-border-radius-bottom">
                        <xsl:apply-templates select="parsepomportlet-portlet" />
                    </div>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="parsepomportlet-portlet">
        <xsl:apply-templates select="parsepomportlet-portlet-content" />
    </xsl:template>

    <xsl:template match="parsepomportlet-portlet-content">
